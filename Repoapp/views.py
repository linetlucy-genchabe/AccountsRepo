...
def signout(request):from django.shortcuts import get_object_or_404, render, redirect
from django.conf import settings
from django.contrib.auth import authenticate, login, logout
from django.contrib.auth.models import User
from django.contrib import messages
...
def index(request):
    ...
    # CountyFocal -> their summary home
    if profile.role == 'CountyFocal':
        return redirect('county_focal_home')
...
def county_focal_home(request):
    ...
    if not counties.exists():
        messages.warning(request, 'You have no assigned county. Please contact your administrator.')
        return render(request, 'county_focal_home.html', {
            'greeting': get_greeting(),
            'county': None,
            'subcounties': [],
            'total_accounts': 0,
    ...
    # Subcounties with account count and CHU count each
    subcounties = Subcounty.objects.filter(subcounty_county=county).annotate(
        account_count=Count('accountnames', distinct=True),
        chu_count=Count('accountnames__Community_Health_Unit', distinct=True),
    ).order_by('name')

    total_accounts = Accounts.objects.filter(account_county=county).count()
    total_subcounties = subcounties.count()
    total_chus = Accounts.objects.filter(account_county=county).values('Community_Health_Unit').distinct().count()
